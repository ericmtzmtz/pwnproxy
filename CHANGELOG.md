...
- **HookBus QoS per subscriber** (`shared/hooks.py`): `HookBus.register` now hands each subscriber its own `QoSClassifiedQueue` classified by the channel's `HOOKBUS_QOS` mapping, replacing the plain per-channel `asyncio.Queue`. Fan-out is preserved because each subscription owns its queue; `_HookChannelQueue.get()` unwraps `(topic, data)` so consumers keep the pre-QoS `await queue.get() -> data` contract. Rationale for per-subscriber queues (vs one queue per channel, which would make subscribers race for events and break fan-out across the ~20 consumers) is documented in the `register()` docstring.
