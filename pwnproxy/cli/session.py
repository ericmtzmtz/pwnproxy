...
console = Console()
app = typer.Typer(help="Manage stored sessions", no_args_is_help=True)
...
async def _list_sessions(token_type: str | None):
    ...
    table = Table(title=f"Sessions ({len(tokens)})")
...
def list(
    ...
    asyncio.run(_list_sessions(type))


@app.command()
def get(token_id: int = typer.Argument(..., help="Session token ID")):
    asyncio.run(_get_session(token_id))


async def _get_session(token_id: int):
    storage = TokenStorage()
    await storage.init()
    ...
    if token is None:
        console.print(f"[red]Session {token_id} not found.[/]")
        raise typer.Exit(1)
    ...
    console.print(Panel(info, title=f"Session #{token.id}"))
...
@app.command()
def delete(token_id: int = typer.Argument(..., help="Session token ID to delete")):
    asyncio.run(_delete_session(token_id))


async def _delete_session(token_id: int):
    storage = TokenStorage()
    await storage.init()
    ...
    console.print(f"[green]Deleted session[/] {token_id}")
