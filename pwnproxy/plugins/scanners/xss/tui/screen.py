...
class XssScannerScreen(Screen[None]):
    ...
    def __init__(self, scanner: XSSScanner, name: str | None = None):
    ...
    def _refresh_table(self) -> None:
        async def reload():
            ...
            if not findings:
                ...
                return
            for f in findings:
                sev_icon = "🔴" if f.severity == "critical" else ("🟠" if (f.severity == "high" and f.confidence == "confirmed") else ("🟡" if f.confidence == "inferred" else "⚪"))
                table.add_row(
                    sev_icon,
                    f.url[:80],
                    f.param_name,
