...
from pwnproxy.services.scanners.xxe.models import Base, XxeFinding

logger = logging.getLogger(__name__)


class XxeFindingStorage:
    ...
    async def save_finding(self, finding: XxeFinding) -> None:
        async with self.session_factory() as session:
            session.add(finding)
            await session.commit()

    async def get_findings(self) -> list[XxeFinding]:
        from sqlalchemy import select
        async with self.session_factory() as session:
            result = await session.execute(
                select(XxeFinding).order_by(XxeFinding.timestamp.desc())
            )
            return list(result.scalars().all())

    async def export_json(self, filepath: Optional[str] = None) -> str:
        if filepath is None:
            filepath = str(Path.home() / ".pwnproxy" / "xxe_findings.json")
