...
class LfiReplayer:
    ...
    async def replay(self, point: InjectionPoint, payload: str, method: str = "") -> Optional[httpx.Response]:
        method = method.upper() if method else point.method.upper()
        ...
        try:
            if point.location == "query":
                ...
                resp = await self._client.request(method, url, headers=headers, content=body, timeout=5.0)
                return resp
                ...
                resp = await self._client.request(method, point.url, headers=headers, content=body, timeout=5.0)
                return resp
            ...
            logger.debug(f"Timeout for {point.url} ({point.name}={payload}) via {method}")
            return None
        except Exception as e:
            logger.warning(f"Replay failed for {point.url} via {method}: {e}")
            return None
